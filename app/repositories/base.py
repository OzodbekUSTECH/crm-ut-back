...
class BaseRepository:
    ...
    async def get_by_email(self, email: str) -> dict:
        ...
        result = await self.session.execute(stmt)
        return result.scalar_one() #dont use to_read_model since it should return all the data of the model cause need to verify password for func authenticate user(in usersService)
